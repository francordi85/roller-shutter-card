...
const GLOBAL_DEFAULTS = {
  ...
  // Layout options
  layout_options: {
    grid_columns: 0,    // 0 = auto, or 1-4 (how many dashboard columns the card spans)
    grid_rows: 0,       // 0 = auto
  },
  card_height: "",      // CSS value: "", "auto", "300px", "50vh", etc.
};
...
class RollerShutterCard extends HTMLElement {
  ...
  constructor() {
    ...
    // CRITICAL: init with full defaults so set hass() never crashes
    this._config = { ...GLOBAL_DEFAULTS };
    this._hass = null;
    this._dragging = null;
  ...
  setConfig(config) {
    ...
    // Always ensure entities is a valid array
  ...
  set hass(hass) {
    this._hass = hass;
    // Only update if setConfig was already called
  ...
  getCardSize() {
    ...
    const cols = this._config.columns || Math.min(entities.length, 4);
    const rows = Math.ceil(entities.length / Math.max(cols, 1));
    return Math.max(2, rows * 3 + 1);
  }

  // HA layout system — tells the dashboard how many grid columns/rows to use
  getLayoutOptions() {
    const lo = this._config.layout_options || {};
    const result = {};
    if (lo.grid_columns && lo.grid_columns > 0) {
      result.grid_columns = lo.grid_columns;
    } else {
      // Auto: suggest full width if 3+ entities
      const count = this._getEntityIds().length;
      result.grid_columns = count >= 3 ? 4 : count >= 2 ? 3 : 2;
    }
    if (lo.grid_rows && lo.grid_rows > 0) {
      result.grid_rows = lo.grid_rows;
    } else {
      result.grid_rows = "auto";
    }
    return result;
  }
  ...
  static getStubConfig(hass) {
    const covers = (hass && hass.states) ? Object.keys(hass.states).filter(e => e.startsWith("cover.")) : [];
    return {
  ...
  _getEntityConfig(entityId) {
    const entry = this._safeEntities().find(
  ...
  _getPosition(entityId) {
    if (!this._hass || !this._hass.states) return 0;
  ...
  _getEntityIds() {
    return this._safeEntities().map((e) => (typeof e === "string" ? e : e.entity));
  }
  ...
  _openCover(entityId) {
    if (this._hass) this._hass.callService("cover", "open_cover", { entity_id: entityId });
  }

  _closeCover(entityId) {
    if (this._hass) this._hass.callService("cover", "close_cover", { entity_id: entityId });
  }

  _stopCover(entityId) {
    if (this._hass) this._hass.callService("cover", "stop_cover", { entity_id: entityId });
  }
  ...
  _render() {
    ...
    const entities = this._getEntityIds();
    const globalW = c.shutter_width || 140;
    const colTemplate = c.columns
      ? `repeat(${c.columns}, 1fr)`
      : `repeat(auto-fill, minmax(${globalW + 50}px, 1fr))`;

    const cardHeight = c.card_height || "";
    const heightStyle = cardHeight ? `height: ${cardHeight};` : "";

    this.shadowRoot.innerHTML = `
      <style>
        :host { display: block; ${heightStyle} }
        .card {
          background: ${c.color_background};
          border-radius: 16px;
          overflow: hidden;
          font-family: var(--ha-card-font-family, 'Segoe UI', Roboto, sans-serif);
          ${heightStyle ? `height: 100%; display: flex; flex-direction: column;` : ""}
        }
        .card-header {
        ...
        .global-btn.open { border-color: rgba(99,179,237,0.25); color: ${c.color_open || "#63b3ed"}; }
        .global-btn.open:hover { background: rgba(99,179,237,0.12); }
        .global-btn.close { border-color: rgba(237,137,54,0.25); color: ${c.color_closed || "#ed8936"}; }
        ...
        .shutters-grid {
          ...
          ${heightStyle ? `flex: 1; align-content: center;` : ""}
        }
  ...
  _renderShutter(entityId) {
    const W = this._resolve(entityId, "shutter_width") || 140;
    const H = this._resolve(entityId, "shutter_height") || 180;
    ...
    const colorOpen = this._resolve(entityId, "color_open") || "#63b3ed";
    const colorClosed = this._resolve(entityId, "color_closed") || "#ed8936";
...
class RollerShutterCardEditor extends HTMLElement {
  ...
  _getEntityObj(entityId) {
    const entry = this._safeEntities().find(e => (typeof e === "string" ? e : e.entity) === entityId);
  ...
  _toggleEntity(entityId, selected) {
    let entities = [...this._safeEntities()];
    if (selected) {
  ...
  _moveEntity(entityId, direction) {
    const entities = [...this._safeEntities()];
  ...
  _updateEntityProp(entityId, key, value) {
    const entities = this._safeEntities().map(e => {
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .opt-field label { display: block; font-size: 12px; margin-bottom: 3px; color: var(--secondary-text-color, #666); }
        ...
        .opt-checks { grid-column: 1 / -1; display: flex; flex-wrap: wrap; gap: 8px 16px; }
        .opt-check { display: flex; align-items: center; gap: 5px; font-size: 12px; color: var(--secondary-text-color); }
        .opt-check input { width: 15px; height: 15px; accent-color: #2d5a87; }
        .opt-hint { grid-column: 1 / -1; font-size: 11px; color: var(--secondary-text-color, #999); font-style: italic; }
        ...
        .no-covers { padding: 20px; text-align: center; color: var(--secondary-text-color, #888); font-style: italic; }
        ...
        .selected-count { font-size: 12px; color: var(--secondary-text-color); padding: 4px 0 8px; }
      </style>
      ...
      <div class="editor">
        ...
        <div class="section">
          ...
          <div class="section-body">
            ...
            ${covers.length === 0
              ? `<div class="no-covers">Aucune entité cover.* trouvée.<br>Vérifiez que vos volets sont configurés dans HA.</div>`
        ...
        <div class="section">
          ...
          <div class="section-body">
            <div class="row">
              <div class="field"><label>Largeur (px)</label><input type="number" id="g_shutter_width" value="${c.shutter_width}" min="80" max="300" /></div>
              <div class="field"><label>Hauteur (px)</label><input type="number" id="g_shutter_height" value="${c.shutter_height}" min="100" max="400" /></div>
            </div>
            <div class="row">
              <div class="field"><label>Colonnes (0 = auto)</label><input type="number" id="g_columns" value="${c.columns}" min="0" max="8" /></div>
              <div class="field"><label>Espacement (px)</label><input type="number" id="g_gap" value="${c.gap}" min="4" max="60" /></div>
            </div>
            <div class="row">
              <div class="field"><label>Couleur ouvert</label><input type="color" id="g_color_open" value="${c.color_open}" /></div>
              <div class="field"><label>Couleur fermé</label><input type="color" id="g_color_closed" value="${c.color_closed}" /></div>
            </div>
        ...
        <div class="section">
          <div class="section-header">📐 Mise en page</div>
          <div class="section-body">
            <div class="row">
              <div class="field">
                <label>Colonnes grille (largeur carte)</label>
                <select id="lo_grid_columns">
                  <option value="0" ${(lo.grid_columns || 0) === 0 ? "selected" : ""}>Auto</option>
                  <option value="1" ${lo.grid_columns === 1 ? "selected" : ""}>1 colonne (petit)</option>
                  <option value="2" ${lo.grid_columns === 2 ? "selected" : ""}>2 colonnes</option>
                  <option value="3" ${lo.grid_columns === 3 ? "selected" : ""}>3 colonnes</option>
                  <option value="4" ${lo.grid_columns === 4 ? "selected" : ""}>4 colonnes (pleine largeur)</option>
                </select>
              </div>
              <div class="field">
                <label>Lignes grille (hauteur carte)</label>
                <select id="lo_grid_rows">
                  <option value="0" ${(lo.grid_rows || 0) === 0 ? "selected" : ""}>Auto</option>
                  <option value="2" ${lo.grid_rows === 2 ? "selected" : ""}>2 lignes</option>
                  <option value="3" ${lo.grid_rows === 3 ? "selected" : ""}>3 lignes</option>
                  <option value="4" ${lo.grid_rows === 4 ? "selected" : ""}>4 lignes</option>
                  <option value="5" ${lo.grid_rows === 5 ? "selected" : ""}>5 lignes</option>
                  <option value="6" ${lo.grid_rows === 6 ? "selected" : ""}>6 lignes</option>
                  <option value="8" ${lo.grid_rows === 8 ? "selected" : ""}>8 lignes</option>
                </select>
              </div>
            </div>
            <div class="field">
              <label>Hauteur fixe de la carte (CSS)</label>
              <input type="text" id="lo_card_height" value="${c.card_height || ""}" placeholder="auto — ex: 400px, 50vh, 100%" />
            </div>
            <div style="font-size:11px;color:var(--secondary-text-color);margin-top:4px;font-style:italic;">
              "Colonnes grille" contrôle la largeur dans le dashboard HA (sections). 4 = pleine largeur.<br>
              "Hauteur fixe" force une hauteur CSS. Les volets seront centrés verticalement.
            </div>
          </div>
        </div>
  ...
  _renderEntityList(covers, selectedIds) {
    const selectedCovers = selectedIds.map(id => covers.find(c => c.entity_id === id)).filter(Boolean);
  ...
  _renderEntityOptions(entityId, entityObj) {
    ...
    return `
      <div class="entity-options" data-options="${entityId}">
        <div class="opt-grid">
          <div class="opt-field"><label>Nom personnalisé</label><input type="text" data-eprop="${entityId}|name" value="${val("name")}" placeholder="Auto" /></div>
          <div class="opt-field"><label>Icône (emoji)</label><input type="text" data-eprop="${entityId}|icon" value="${val("icon")}" placeholder="🪟" /></div>
          <div class="opt-field"><label>Largeur (px)</label><input type="number" data-eprop="${entityId}|shutter_width" value="${val("shutter_width")}" placeholder="${placeholder("shutter_width")}" min="80" max="300" /></div>
          <div class="opt-field"><label>Hauteur (px)</label><input type="number" data-eprop="${entityId}|shutter_height" value="${val("shutter_height")}" placeholder="${placeholder("shutter_height")}" min="100" max="400" /></div>
          <div class="opt-field"><label>Couleur ouvert</label><input type="color" data-eprop="${entityId}|color_open" value="${val("color_open") || c.color_open}" /></div>
          <div class="opt-field"><label>Couleur fermé</label><input type="color" data-eprop="${entityId}|color_closed" value="${val("color_closed") || c.color_closed}" /></div>
  ...
  _eCheck(entityId, key, label, entityObj) {
    ...
    const checked = localVal !== undefined ? localVal : globalVal;
    return `<div class="opt-check"><input type="checkbox" data-echeck="${entityId}|${key}" ${checked ? "checked" : ""} /><label>${label}</label></div>`;
  }
  ...
  _attachEditorEvents() {
    ...
    $$(".entity-cb").forEach(cb => {
      cb.addEventListener("change", () => this._toggleEntity(cb.dataset.entity, cb.checked));
    });
    ...
    const globalFields = { g_shutter_width: "shutter_width", g_shutter_height: "shutter_height", g_columns: "columns", g_gap: "gap", g_color_open: "color_open", g_color_closed: "color_closed" };
    for (const [elId, key] of Object.entries(globalFields)) {
      const el = $(`#${elId}`);
      if (el) el.addEventListener("change", () => this._updateGlobal(key, el.type === "number" ? (parseInt(el.value) || 0) : el.value));
    }

    for (const key of ["show_buttons", "show_percentage", "show_global_controls", "show_drag_hint", "show_sky", "show_stars", "show_window_frame", "invert_position"]) {
      const el = $(`#g_${key}`);
      if (el) el.addEventListener("change", () => this._updateGlobal(key, el.checked));
    }

    // Layout options
    const loGridCols = $("#lo_grid_columns");
    const loGridRows = $("#lo_grid_rows");
    const loCardHeight = $("#lo_card_height");

    if (loGridCols) {
      loGridCols.addEventListener("change", () => {
        const lo = { ...(this._config.layout_options || {}), grid_columns: parseInt(loGridCols.value) || 0 };
        this._updateGlobal("layout_options", lo);
      });
    }
    if (loGridRows) {
      loGridRows.addEventListener("change", () => {
        const lo = { ...(this._config.layout_options || {}), grid_rows: parseInt(loGridRows.value) || 0 };
        this._updateGlobal("layout_options", lo);
      });
    }
    if (loCardHeight) {
      loCardHeight.addEventListener("change", () => {
        this._updateGlobal("card_height", loCardHeight.value.trim());
      });
    }
  }
}
